<template>
<div :class="'weui_dialog_' + (type === 'alert'?'alert':'confirm') + (show?' weui_dialog_show':'')">
<!--  <div class="weui_mask" v-show="show"></div>-->
  <div class="weui_dialog animated" v-show="show" transition="zoom">
    ...
    <div class="weui_dialog_ft">
      <a v-if="type !== 'alert'" href="javascript:;" class="weui_btn_dialog default" @click="dispathEventAndClose('weui-dialog-cancel')">{{cancelButton}}</a>
...
export default {
  ...
  methods: {
    dispathEventAndClose(event) {
      this.$dispatch(event);
      if(this.type === 'alert' || event === 'weui-dialog-cancel'){
        this.show = false;
      }
    }
  }
